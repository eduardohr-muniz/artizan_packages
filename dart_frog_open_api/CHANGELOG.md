...
- **Breaking:** the sorting/expansion options are now type-safe enums instead of raw
  strings — `swaggerUiHandler(docExpansion:)` takes `SwaggerDocExpansion`
  (`.none` / `.list` / `.full`), and `ScalarOptions.tagsSorter` /
  `ScalarOptions.operationsSorter` take `ScalarTagsSorter` / `ScalarOperationsSorter`

## [0.1.4]

### Added

- `ScalarOptions.tagsSorter` (default `'alpha'`) and `ScalarOptions.operationsSorter`
  so the Scalar sidebar is alphabetized regardless of spec order — Scalar does not
  honour the top-level `tags` array ordering on its own
- `docExpansion` parameter on `swaggerUiHandler` (`'none'` / `'list'` / `'full'`)
  to control whether Swagger UI sections start collapsed or expanded
